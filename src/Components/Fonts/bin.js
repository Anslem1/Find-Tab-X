...
function Bin ({ onHoverTitle, closeTabs, tabsToDeleteId, deleteSelectedTab }) {
  return (
    <svg
      ...
      onMouseOver={() => {
        onHoverTitle(
          tabsToDeleteId.length > 0
            ? `Delete ${tabsToDeleteId.length} selected tabs`
            : 'Delete currently active tab'
        )
      }}
      onMouseLeave={() => {
        onHoverTitle('')
      ...
      className='font red'
      onClick={tabsToDeleteId.length > 0 ? deleteSelectedTab : closeTabs}
    >
      <title>
        {tabsToDeleteId.length > 0
